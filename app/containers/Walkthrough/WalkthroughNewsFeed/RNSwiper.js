...
export default class SwiperContainer extends Component {
  ...
  componentWillMount() {
    ...
    this.state.containerOffset.addListener(
    ...
    this._panResponder = PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (
          gestureState.dx > this.swipeThreshold &&
          (this.state.activeX > 0 ||
            (this.state.activeX > 1 && !animationComplete))
        ) {
        ...
        //swipe left if swipe threshold is reached
          ...
          gestureState.dx < this.swipeThreshold * -1 &&
          this.state.activeX < this.state.cards.length - 1
        ) {
          if (!this.state.swipeRegistered) {
            this.setState({
              swipeRegistered: true,
        ...
        // handle touch responsiveness before final animation is called
          ...
          if (
            ...
            this.state.containerOffset.setValue(
              this.state.containerOffset._value + dx / 80
            );
          }
          if (dx > 0) {
            this.state.opacityToAppearLeft.setValue(
              this.minimumOpacity + valToSet
            );
            ...
            this.state.scaleToGrowRight.setValue(
              this.minimumScale + valToSet * -1
            );
            this.state.opacityToFade.setValue(
              this.minimumOpacity + valToSet * -1
            );
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (this.state.animationComplete) {
          ...
          Animated.parallel(
            [
              Animated.timing(this.state.containerOffset, {
                toValue:
                  -1 *
                  (this.state.activeX * (this.cardWidth + this.overlap * -1) -
                    this.margin),
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.scaleToGrowLeft, {
                toValue: this.minimumScale,
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.scaleToShrink, {
                toValue: 1,
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.opacityToAppearLeft, {
                toValue: this.minimumOpacity,
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.opacityToFade, {
                toValue: 1,
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.scaleToGrowRight, {
                toValue: this.minimumScale,
                duration: this.duration,
                easing: Easing.easeInOut
              }),
              Animated.timing(this.state.opacityToAppearRight, {
                toValue: this.minimumOpacity,
                duration: this.duration,
                easing: Easing.easeInOut
              })
  ...
  swipeLeft() {
    Animated.parallel(
      [
        Animated.timing(this.state.containerOffset, {
          toValue:
            this.state.containerOffset._value -
            (this.cardWidth + this.overlap * -1),
          duration: this.duration,
          easing: Easing.easeInOut
        }),
        Animated.timing(this.state.scaleToGrowRight, {
  ...
  swipeRight() {
    Animated.parallel(
      ...
      this.setState(
        {
          activeX: this.state.activeX + activeToAdd,
          animationComplete: true,
          zIndex: [3, 2, 1, 0]
        },
        () => {
  ...
  render() {
    return (
      <Animated.View
        style={[styles.container, { marginLeft: this.state.containerOffset }]}
          ...
          if (this.state.activeX === i) {
            return (
              <View style={{ zIndex: this.state.zIndex[0] }} key={i}>
                <Animated.View
                  ...
                  style={[
                    {
                      marginRight: this.overlap * 0.5,
                      zIndex: this.state.zIndex[0],
                      width: this.props.cardWidth,
                      opacity: this.state.opacityToFade,
                      transform: [{ scale: this.state.scaleToShrink }]
                    }
                  ]}
                >
                  {card}
            ...
            return (
              <Animated.View
                ...
                style={[
                  {
                    marginLeft: this.overlap * -1,
                    marginRight: this.overlap * 0,
                    zIndex: this.state.zIndex[1],
                    width: this.props.cardWidth,
                    opacity: this.state.opacityToAppearRight,
                    transform: [{ scale: this.state.scaleToGrowRight }]
                  }
                ]}
              >
                {card}
            ...
            return (
              <Animated.View
                ...
                style={[
                  {
                    marginRight: this.overlap * -1,
                    zIndex: this.state.zIndex[2],
                    width: this.props.cardWidth,
                    opacity: this.state.opacityToAppearLeft,
                    transform: [{ scale: this.state.scaleToGrowLeft }]
                  }
                ]}
              >
                {card}
                ...
                style={[
                  {
                    ...
                    width: this.props.cardWidth,
                    opacity: this.minimumOpacity,
                    transform: [{ scale: this.minimumScale }]
                  }
                ]}
              >
                {card}
